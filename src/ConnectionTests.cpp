...
//     size_t maxPackNum;
//     bool allDone;
//     auto metaSizeOwn = metaInfoReceive.size() / 2;
...
//                 allDone &= (metaInfoReceive[c + metaSizeOwn] == rdma_ready);
//             }
//         }
//     }
...
//             setOpcode((metaInfoReceive.size() / 2) + nextFree, rdma_sending, false);
...
//             setOpcode((metaInfoReceive.size() / 2) + nextFree, rdma_data_finished, true);
...
//         while (metaInfoReceive[(metaInfoReceive.size() / 2)] != rdma_ready) {
//             using namespace std::chrono_literals;
//             std::this_thread::sleep_for(10ms);
//         }

//         setOpcode(metaInfoReceive.size() / 2, rdma_multi_thread, true);
...
//                                 setOpcode(id + (metaInfoReceive.size() / 2), rdma_sending, false);
...
//                                 setOpcode(id + (metaInfoReceive.size() / 2), rdma_ready, false);
...
//                             while (!packageSent) {
//                                 for (size_t id = tid; id < bufferConfig.num_remote_receive; id += thrdcnt) {
//                                     size_t remoteIndex = id + (metaInfoReceive.size() / 2);
//                                     if (metaInfoReceive[remoteIndex] == rdma_ready) {
...
//             setOpcode(c + (metaInfoReceive.size() / 2), rdma_test_finished, true);
//         }
//     }
